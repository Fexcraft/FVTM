	...
	public InteractionResult use(BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit){
		...
		if(!player.isCrouching() && type.getFunctions().size() > 0){
			for(BlockFunction func : type.getFunctions()){
				if(func.onClick(getWorld(level), getPos(pos), new V3D(0, 0, 0), StateWrapper.of(state), getSide(hit.getDirection()), UniEntity.getCasted(player), hand == InteractionHand.MAIN_HAND)) return InteractionResult.SUCCESS;
			}
		}
