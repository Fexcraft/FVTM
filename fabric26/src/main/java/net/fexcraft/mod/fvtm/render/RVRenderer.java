...
public class RVRenderer extends EntityRenderer<RootVehicle, VehicleRenderState> {
	...
	public void submit(VehicleRenderState state, PoseStack pose, SubmitNodeCollector noco, CameraRenderState camera){
		...
		state.vehicle.data.renderdata().update(state.vehicle, state.f).light(state.lightCoords);
		sepcache = state.vehicle.rendercache().get(SEP_VEH_CACHE, data -> new SeparateRenderCache.SepVehCache());
		pose.pushPose();
		...
		//
		RenderUtil26.set(pose, noco, FvtmRenderTypes.getCutout(state.vehicle.data.getCurrentTexture()), state.lightCoords);
		pose.pushPose();
		Model vehmod = state.vehicle.data.getType().getModel();
		if(vehmod != null){
			pose.pushPose();
			RENDER_UTIL.render(vehmod, state.vehicle.data.renderdata);
			pose.popPose();
		}
		else{
		...
		if(state.vehicle.data.getParts().size() > 0){
			renderPoint(pose, state.vehicle.point, state.vehicle.data, state.f);
		}
		...
		pose.popPose();
		renderDetachedPoints(pose, state.entity, state.vehicle.data, state.f);
		if(Config.DEBUG_ACTIVE){
			Renderer26.light = LightCoordsUtil.FULL_BRIGHT;
			...
			for(OBB.OBBRef ref : state.vehicle.data.getBoundBoxes()){
				...
				for(V3D vert : obb.verts){
					...
					RenderUtil26.renderSphere(0.025f, COL_CYN);
					pose.popPose();
				}
			}
		}
	...
	private void renderSeats(PoseStack pose, VehicleInstance vehicle){
		...
		for(SeatInstance seat : vehicle.seats){
			...
			RenderUtil26.renderBB(pos, scale, COL_YLW);
		}
		pose.popPose();
	}
	...
	public static void renderVehicleInfo(PoseStack pose, V3D vehpos, VehicleData data){
					...
					for(String str : part.getType().getCategories()){
						if(str.equals(type)){
							...
							if(point.isVehicle()){
								...
								RENDERER.rotate(point.getPivot().deg_yaw(), 0, 1, 0);
		...
		if(tool > -1){
			...
			for(WheelSlot slot : data.getWheelSlots().values()){
				RenderUtil26.renderBB(slot.position, slot.max_radius, red ? COL_RED : COL_CYN);
			}
		}
		part = isWheelOrTire();
		if(part != null){
			...
			for(Map.Entry<String, WheelSlot> entry : data.getWheelSlots().entrySet()){
				...
				slot = entry.getValue();
				RenderUtil26.renderBB(slot.position, slot.max_radius, red ? COL_RED : green ? COL_GRN : COL_CYN);
			}
		}
		part = isBogie();
		if(part != null){
		...
		if(trai != null && trai.getType().isTrailer()){
			for(Map.Entry<String, V3D> entry : data.getConnectors().entrySet()){
				...
				RenderUtil26.renderBB(entry.getValue(), 0.8f, red ? COL_RED : COL_GRN);
			}
		}
		//
		tool = isToolbox();
	...
	public static void renderPoint(PoseStack pose, SwivelPoint point, VehicleData data, float ticks){
		...
		for(Map.Entry<String, PartData> entry : parts){
			...
			RENDER_UTIL.render(entry.getValue().getType().getModel(), entry.getValue().renderdata().update(data.renderdata, entry.getKey(), ticks).light(data.renderdata.light));
			pose.popPose();
		}
	...
	public static void renderPointSep(PoseStack pose, SwivelPoint point, VehicleInstance inst, ArrayList<String> parts, float ticks){
		...
		for(Map.Entry<String, PartData> entry : inst.data.getParts().entrySet()){
			...
			rotate(pose, entry.getValue().getInstalledRot());
			RenderUtil26.type(FvtmRenderTypes.getCutout(entry.getValue().getCurrentTexture()));
			RENDER_UTIL.render(entry.getValue().getType().getModel(), entry.getValue().renderdata().update(inst.data.renderdata, entry.getKey(), ticks).light(inst.data.renderdata.light).sep());
			pose.popPose();
		}
