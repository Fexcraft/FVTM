...
public class VehicleCatalogImpl extends VehicleCatalog {
    ...
    public void postdraw(float ticks, int mx, int my){
		...
		if(data == null) return;
		rv.initVD(data);
        GuiGraphicsExtractor gg = root.matrix();
		...
		gg.entity(renderer.createRenderState(rv, 1f), preset.scale * 16, new Vector3f(), qua, null, gLeft + 7, gTop + 23, gLeft + 127, gTop + 103);
		gg.disableScissor();
    }
