...
public class Resources21 extends FvtmResources {
	...
	public void createContentItems(){
		...
		FvtmRegistry.WIRE_COMPS.forEach(wire -> wire.setItemWrapper(wrapwrapper(wire.getID(), FVTM.regItem(wire.getIDS(), prop -> new WireCompItem(prop, wire)))));
	...
	public ItemWrapper getItemWrapper(String id){
		Optional<Holder.Reference<Item>> item = BuiltInRegistries.ITEM.get(Identifier.parse(id));
