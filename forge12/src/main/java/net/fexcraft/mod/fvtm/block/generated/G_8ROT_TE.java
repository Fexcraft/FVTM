...
public class G_8ROT_TE extends BlockBase {

    public G_8ROT_TE(Block type){
        super(type); this.setDefaultState(this.blockState.getBaseState().withProperty(ROTATION8, 0));
    }
    ...
    public IBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer){
        return this.getDefaultState().withProperty(ROTATION8, type.isRandomRot() ? Static.random.nextInt(8) : MathHelper.floor((double)(placer.rotationYaw * 9f / 360f) + 0.5d) & 7);
    }

    @Override
    public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack){
        super.onBlockPlacedBy(world, pos, state, placer, stack);
        world.setBlockState(pos, state.withProperty(ROTATION8, type.isRandomRot() ? Static.random.nextInt(8) : MathHelper.floor((double)(placer.rotationYaw * 8f / 360f) + 0.5d) & 7), 2);
    }
