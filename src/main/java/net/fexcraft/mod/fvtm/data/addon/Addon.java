...
public class Addon extends TypeCore<Addon> {
	...
	public Addon parse(JsonObject obj){
		...
		if(obj.has("ClothMaterials")){
			obj.get("ClothMaterials").getAsJsonObject().entrySet().forEach(entry -> {
				...
				//TODO armor materials armats.put(entry.getKey(), EnumHelper.addArmorMaterial(entry.getKey(), Resources.NULL_TEXTURE.toString(), durr, ams, 0, SoundEvents.ITEM_ARMOR_EQUIP_LEATHER, tgh));
				Print.log("Cloth Materials are not re-implemented yet. Found entry '" + entry.getKey() + "'.");
			});
		}
		...
		if(obj.has("WireDecos")){
			//TODO wire deco //Resources.WIRE_DECO_CACHE.put(this.getRegistryName().getPath(), obj.get("WireDecos").getAsJsonObject()
			Print.log("Wire Decos are not re-implemented yet. Found entries in'" + this.getRegistryName() + "'.");
		}
		if(obj.has("Particles") && Static.isClient()){
			...
			for(Entry<String, JsonElement> entry : par.entrySet()){
				//TODO particles // new net.fexcraft.mod.fvtm.sys.particle.Particle(registryname.getPath() + ":" + entry.getKey(), JsonHandler.parse(entry.getValue().toString(), true).asMap());
				Print.log("Particles are not re-implemented yet. Found entry '" + entry.getKey() + "'.");
			}
		}
		if(obj.has("Conditions")){
			...
			for(Entry<String, JsonElement> entry : par.entrySet()){
				...
				ConditionRegistry.register(cond);*///TODO Conditions
				Print.log("Conditions are not re-implemented yet. Found entry '" + entry.getKey() + "'.");
			}
		}
		if(obj.has("TrafficSigns")){
			...
			lib.load();*///TODO traffic signs
			Print.log("Traffic Signs are not re-implemented yet. Found entries in '" + this.getRegistryName() + "'.");
		}
	...
	public File getZipFile(){
		return contype.zip() ? configs : null;
	}
	...
	public void searchFor(DataType data) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		...
		if(contype == ContainerType.DIR){
			if(!loc.isMDK() && !configs.isDirectory()) return;
			//
			File folder = new File(configs, "assets/" + registryname.getPath() + "/config/" + data.cfg_folder + "/");
			if(!folder.exists()){ folder.mkdirs(); }
			ArrayList<File> candidates = findFiles(folder, data.suffix);
			for(File file : candidates){
				JsonObject obj = JsonUtil.get(file);
				TypeCore<?> core = (TypeCore<?>)data.core.getConstructor().newInstance().parse(obj);
				if(core == null){
			...
			JsonArray array = ZipUtil.getJsonObjectsAt(configs, "assets/" + registryname.getPath() + "/config/" + data.cfg_folder + "/", data.suffix);
	...
	private void checkLangFile(TypeCore<?> core){
		...
		}*///TODO check for missing lang entries
	}
	...
	private void checkItemJson(TypeCore<?> core, DataType data){
		...
		//TODO generate missing item icon
	...
	private void checkItemIcon(TypeCore<?> core, DataType data){
		...
		}*///TODO load item placeholder icon
	}
	...
	public void loadPresets(){
		...
		//TODO preset loading
		Print.log("Preset loading is not re-implemented yet.");
		/*if(contype == ContainerType.DIR){
	...
	public void loadRecipes(){
		...
		//TODO recipe loading
		Print.log("Recipe loading is not re-implemented yet.");
		/*if(contype == ContainerType.DIR){
			if(!file.isDirectory()) return;
			//
