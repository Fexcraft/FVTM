...
public class AdjSignGui {
    ...
    public static class Client extends GuiContainer {
        ...
        public Client(EntityPlayer player, World world, int x, int y, int z){
            ...
            if(tile instanceof AdjustableSignEntity){
                this.adjsign = (AdjustableSignEntity) tile;
            }
            if(tile instanceof CylinderSignEntity){
                this.cylsign = (CylinderSignEntity) tile;
            }
            this.xSize = 254;
            this.ySize = 56;
            this.player = player;
        ...
        public void actionPerformed(GuiButton button){
            ...
            if(adjsign == null){
                switch(button.id){
                    case 2: {
                        cylsign.rescale(cylsign.getScale() - 1);
                        break;
                    }
                    case 3: {
                        cylsign.rescale(cylsign.getScale() + 1);
                    ...
                    case 4: {
                        cylsign.segments(cylsign.getSegments() - 1);
                        break;
                    }
                    case 5: {
                        cylsign.segments(cylsign.getSegments() + 1);
                ...
                switch(button.id){
                    case 2: {
                        adjsign.resize(EnumFacing.Plane.HORIZONTAL, adjsign.getWidth() - 1);
                        break;
                    }
                    case 3: {
                        adjsign.resize(EnumFacing.Plane.HORIZONTAL, adjsign.getWidth() + 1);
                    ...
                    case 4: {
                        adjsign.resize(EnumFacing.Plane.VERTICAL, adjsign.getWidth() - 1);
                        break;
                    }
                    case 5: {
                        adjsign.resize(EnumFacing.Plane.VERTICAL, adjsign.getWidth() + 1);
