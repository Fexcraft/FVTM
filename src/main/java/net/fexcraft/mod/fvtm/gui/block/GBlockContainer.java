...
public class GBlockContainer extends GenericContainer {
	...
	public void detectAndSendChanges(){
		if(fluid_io != null && invtype.isFluid()){
			if(fluid_date + 50 <= Time.getDate()){
				...
				if(!stack.isEmpty() && stack.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY, null)){
					...
					if(item.getTankProperties().length > 0 && item.getTankProperties()[0].getContents() != null && item.getTankProperties()[0].getContents().amount > 0){
						FluidActionResult result = FluidUtil.tryEmptyContainer(stack, tile.getMultiBlockData().getFluidTank(inv_id), 1000, player, true);
						if(result.success){
							anychange = true;
				...
				//
				if(tile.getMultiBlockData().getFluidTank(inv_id).getFluidAmount() > 0){
					stack = fluid_io.getStackInSlot(1);
					if(!stack.isEmpty() && stack.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY, null)){
						FluidActionResult result = FluidUtil.tryFillContainer(stack, tile.getMultiBlockData().getFluidTank(inv_id), 1000, player, true);
						if(result.success){
							anychange = true;
