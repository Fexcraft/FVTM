...
public class GBlockInventory extends GenericGui<GBlockContainer> {
	...
	protected void init(){
		switch(container.invtype){
			...
			case ITEM:{
				...
				texts.put("page", new BasicText(guiLeft + 188, guiTop + 138, 40, MapColor.SNOW.colorValue, "1/" + (container.tile.getMultiBlockData().getInventory(container.inv_id).size() / 78 + 1)));
	...
	protected void predraw(float pticks, int mouseX, int mouseY){
		if(container.fluid_io != null){//assume we're in fluid mode
			FluidTank tank = container.tile.getMultiBlockData().getFluidTank(container.inv_id);
	...
	private void updatePage(int i){
		if(!container.invtype.isItem()) return;
		container.page += i; if(container.page < 0) container.page = 0; container.populateSlots();
		texts.get("page").string = (container.page + 1) + "/" + (container.tile.getMultiBlockData().getInventory(container.inv_id).size() / 78 + 1);
