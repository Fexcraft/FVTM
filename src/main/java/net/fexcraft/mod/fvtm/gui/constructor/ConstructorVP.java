...
public class ConstructorVP extends ConstructorGui {
	...
	private static RGB current, org;
	...
	public void init(){
		...
		this.updateColorTo(colorable.getColorChannel(channel), true);
		org = colorable.getColorChannel(channel).copy();
	}
	...
	protected boolean buttonClicked(int mouseX, int mouseY, int mouseButton, String key, BasicButton button){
		...
		else if(button.name.equals("icon_type_prev") || button.name.equals("icon_type_next")){
			Colorable colorable = getColorable();
			int index = colorable.getColorChannels().headMap(channel).size();
			index += button.name.equals("icon_type_prev") ? -1 : 1;
			if(index < 0) index = colorable.getColorChannels().size() - 1;
			if(index >= colorable.getColorChannels().size()) index = 0;
			channel = colorable.getColorChannels().keySet().toArray(new String[0])[index];
			org = colorable.getColorChannel(channel).copy();
			return true;
		}
		else if(button.name.equals("spectrum")){
			this.updateColorTo(spectrum.getColorAt(mouseX), true);
			return true;
		}
		else if(button.name.equals("palette")){
			this.updateColorTo(palette.getColorAt(mouseX, mouseY), false);
			return true;
		}
		else if(button.name.equals("icon_rgb")){
			this.updateColorTo(tryParse(rgb.getText(), false), true);
			return true;
		}
		else if(button.name.equals("icon_hex")){
			this.updateColorTo(tryParse(hex.getText(), true), true);
			return true;
		}
		else if(button.name.equals("icon_remove")){
			this.updateColorTo(org, true);
			sendColorUpdate();
			return true;
		}
		else if(button.name.equals("icon_check")){
			sendColorUpdate();
			return true;
		}
		return true;
	...
	private RGB tryParse(String text, boolean hex){
		...
		try{
			if(hex){
				rgb = new RGB();
				rgb.packed = Integer.parseInt(text.replace("#", ""), 16);
			}
			else{
