...
public class UniVarInvContainer extends GenericContainer {
	...
	public UniVarInvContainer(EntityPlayer player, World world, int ID, int x, int y, int z){
		...
		if(ID == GuiHandler.MULTIBLOCK_INVENTORY_VAR){
			...
			invhandler = (InvHandlerVar)blk_tile.getBlockData().getFunctionInventory().inventory();
			...
			invhandler = (InvHandlerVar)func.inventory();
