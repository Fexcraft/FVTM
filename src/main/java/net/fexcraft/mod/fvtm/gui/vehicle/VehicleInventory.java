...
public class VehicleInventory extends GenericGui<VehicleContainer> {
	...
	protected void predraw(float pticks, int mouseX, int mouseY){
		if(container.fluid_io != null){//assume we're in fluid mode
			FluidTank tank = container.function.getFluidTank();
	...
	protected void drawbackground(float pticks, int mouseX, int mouseY){
		...
		if(container.fluid_io != null){
			float percent = container.function.getFluidTank().getFluidAmount() * 1f / container.function.getFluidTank().getCapacity() * 100f;
	...
	private void updatePage(int i){
		if(!container.function.isInventoryType(InventoryType.ITEM)) return;
