...
public class WireUnit {
	...
	private void updateClient(){
		...
		if(orig == null && copy == null){
			...
			PacketHandler.getInstance().sendToAllAround(new PacketNBTTagCompound(compound), Resources.getTargetPoint(wire.relay.holder.region.system.getDimension(), wire.start.pos));
			if(wire.length > 16){
				PacketHandler.getInstance().sendToAllAround(new PacketNBTTagCompound(compound), Resources.getTargetPoint(wire.relay.holder.region.system.getDimension(), wire.end.pos));
			}
		}
	}
