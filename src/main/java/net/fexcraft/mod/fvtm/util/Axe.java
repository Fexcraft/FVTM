...
public class Axe {
    ...
    public Axe(Matrix4f mat){
        matrix = mat;
        convertMatrixToAngles();
    }

    public Axe(float yaw, float pitch, float roll){
    ...
    public Vec3f getRelativeVector(Vec3f vec){
        ...
        Matrix4f.rotate((roll  * 3.14159265F / 180f), new Vector3f(1F, 0F, 0F), mat, mat);
        Matrix4f.rotate((pitch * 3.14159265F / 180f), new Vector3f(0F, 0F, 1F), mat, mat);
        Matrix4f.rotate((yaw   * 3.14159265F / 180f), new Vector3f(0F, 1F, 0F), mat, mat);
        return new Vec3f(mat.m00, mat.m10, mat.m20);
    }

	public Vec3f getRelativeVector(float x, int y, int z){
    ...
    public Axe getRelativeVector(Axe axes){
        Matrix4f mat = new Matrix4f();
    ...
    public NBTTagCompound write(Entity ent, NBTTagCompound compound){
        ...
        compound.setFloat("roll", roll);
        return compound;
    }

    public static Axe read(Entity ent, NBTTagCompound nbt){
        Axe axes = new Axe();
        axes.yaw = nbt.getFloat("yaw");
        axes.pitch = nbt.getFloat("pitch");
        axes.roll = nbt.getFloat("roll");
        if(ent != null){
