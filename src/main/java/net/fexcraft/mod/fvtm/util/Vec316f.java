...
public class Vec316f implements Comparable<Vec316f> {
	...
	public Vec316f(Level world, Vec3 pos, int rgs){
		this(validatePos(world, pos), rgs);
	}

	private static Vec3 validatePos(Level world, Vec3 pos){
		if(world != null){
			BlockState state = world.getBlockState(new BlockPos(pos));
			if(state.getBlock() instanceof RailBlock) pos = new Vec3(pos.x, (int)pos.y, pos.z);
		}
		return pos;
	}

	public Vec316f(Vec3 pos, int rgs){
	...
	public Vec316f(CompoundTag compound){
		pos = BlockPos.of(compound.getLong("vec_pos"));
	...
	public CompoundTag write(){
		...
		compound.putLong("vec_pos", pos.asLong());
		compound.putByte("pos_x", x);
		compound.putByte("pos_y", y);
		compound.putByte("pos_z", z);
		return compound;
	}
