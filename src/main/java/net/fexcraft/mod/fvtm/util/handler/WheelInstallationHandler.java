...
public class WheelInstallationHandler extends PartInstallationHandler {
	...
	public boolean allowInstall(@Nullable ICommandSender sender, PartData part, String cat, VehicleData data){
		...
		if(idata.radius > slot.maxradius()){
			Print.chatnn(sender, "Wheel radius too large. [" + idata.radius + ">" + slot.maxradius() + "]");
			return false;
		}
		if(idata.radius < slot.minradius()){
			Print.chatnn(sender, "Wheel radius too small. [" + idata.radius + "<" + slot.minradius() + "]");
			return false;
		}
		if(idata.width > slot.maxwidth()){
			Print.chatnn(sender, "Wheel is too wide. [" + idata.width + ">" + slot.maxwidth() + "]");
			return false;
		}
		if(idata.width < slot.minwidth()){
			Print.chatnn(sender, "Wheel is too thin. [" + idata.width + "<" + slot.minwidth() + "]");
			return false;
		}
